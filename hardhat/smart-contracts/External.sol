...
contract External {
  ...
  function revokeDelegationAddressUsingSubdelegation(
    ...
    if (delegatorHashes[delegatorHash].length > 0) {
      ...
      for (
        ...
        if (
          _delegatorAddress == delegationAddressHashes[delegationAddressHash][i]
        ) {
          countDA = countDA + 1;
        }
  ...
  function retrieveCollectionUseCaseLockStatus(
    ...
    if (_useCase == 1) {
      return
        retrieveCollectionLockStatus(_collectionAddress, _delegationAddress);
    } else {
      bytes32 collectionUsecaseLockHash;
      collectionUsecaseLockHash = keccak256(
        abi.encodePacked(_collectionAddress, _delegationAddress, _useCase)
      ...
      for (uint256 y = 0; y < k; ) {
        if (globalDelegationHashes[allGlobalHashes[y]].length > 0) {
          for (
            ...
            allExpirations[count2] = globalDelegationHashes[allGlobalHashes[y]][
              w
            ].expiryDate;
            allTokens[count2] = globalDelegationHashes[allGlobalHashes[y]][w]
              .allTokens;
            tokensIDs[count2] = globalDelegationHashes[allGlobalHashes[y]][w]
              .tokens;
            count2 = count2 + 1;
