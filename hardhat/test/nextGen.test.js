...
describe("NFTDelegation tests", function () {
  ...
  context("Register Delegation", () => {
    ...
    it("#checkDelegationStatus", async function () {
      const status =
        await contracts.hhDelegation.retrieveGlobalStatusOfDelegation(
  ...
  context("Register Multiple Delegations", () => {
    ...
    it("#retrieveDelegationAddresses", async function () {
      const delegationAddresses =
        await contracts.hhDelegation.retrieveDelegationAddresses(
          signers.owner.address,
          "0x8888888888888888888888888888888888888888",
          "1"
        );
      expect(delegationAddresses[0]).equal(
        "0x03C6FcED478cBbC9a4FAB34eF9f40767739D1Ff7"
      );
    });
    ...
    it("#retrieveDelegationAddresses", async function () {
      ...
      expect(delegationAddresses[1]).equal(
        "0x5B38Da6a701c568545dCfcB03FcB875f56beddC4"
      );
    });
    ...
    it("#retrieveDelegators", async function () {
      const delegatorAddresses =
        await contracts.hhDelegation.retrieveDelegators(
          "0x03C6FcED478cBbC9a4FAB34eF9f40767739D1Ff7",
          "0x8888888888888888888888888888888888888888",
          "1"
        );
  ...
  context("batch delegation", () => {
    // register batch delegation
    it("#batchDelegations", async function () {
      await contracts.hhDelegation.batchDelegations(
        [
          "0x8888888888888888888888888888888888888888",
          "0x8888888888888888888888888888888888888888",
        ],
        [signers.addr1.address, signers.addr2.address],
        ["1799339953", "1799339953"],
        ["1", "1"],
        ["true", "true"],
        ["0", "0"]
      );
    ...
    // register external batch delegation
    it("#batchDelegationsExternal", async function () {
      await contracts.hhExternal.batchDelegations(
        [
          "0x8888888888888888888888888888888888888888",
          "0x8888888888888888888888888888888888888888",
        ],
    ...
    // register batch delegation
    it("#batchRevocations", async function () {
      await contracts.hhDelegation.batchRevocations(
        [
          "0x8888888888888888888888888888888888888888",
          "0x8888888888888888888888888888888888888888",
        ],
    ...
    // register external batch delegation
    it("#batchRevocationsExternal", async function () {
      await contracts.hhExternal.batchRevocations(
        [
          "0x8888888888888888888888888888888888888888",
          "0x8888888888888888888888888888888888888888",
        ],
  ...
  context("Register duplicate Delegations", () => {
    // register a delegation from addr1 to addr2
    it("#registerDelegation", async function () {
      await contracts.hhDelegation
        .connect(signers.addr1)
        .registerDelegationAddress(
    ...
    // register the same delegation, creating a duplicate
    it("#registerDelegation", async function () {
      await contracts.hhDelegation
        .connect(signers.addr1)
        .registerDelegationAddress(
    ...
    // create a batch delegation, to bring the total delegations to 4
    it("#batchDelegationsr", async function () {
      await contracts.hhDelegation
        .connect(signers.addr1)
        .batchDelegations(
          [
          ...
          [signers.addr2.address, signers.addr2.address],
          ["1799339953", "1799339953"],
          ["1", "1"],
          ["true", "true"],
          ["0", "0"]
    ...
    // check the duplicates aren't being removed properly
    it("#retrieveDelegationAddresses", async function () {
      const addr1Delegations =
        await contracts.hhDelegation.retrieveDelegationAddressesTokensIDsandExpiredDates(
          signers.addr1.address,
          "0x8888888888888888888888888888888888888888",
          "1"
        );

      expect(addr1Delegations[0].length === 4).equal(true);
    });
  }); // end of register context

  context("Exploit global delegations", () => {
    // register a delegation from addr1 to addr2
    it("#registerDelegation", async function () {
      await contracts.hhVuln
        .connect(signers.addr1)
        .registerDelegationAddress(
          "0x8888888888888888888888888888888888888888",
          signers.addr2.address,
    ...
    // check the status of the delegation
    it("#checkDelegationStatus", async function () {
      const status = await contracts.hhVuln.retrieveGlobalStatusOfDelegation(
        signers.addr1.address,
        "0x8888888888888888888888888888888888888888",
        signers.addr2.address,
        "1"
    ...
    // check the duplicates are being removed properly
    it("#retrieveDelegationAddresses", async function () {
      await expect(
        contracts.hhVuln.retrieveDelegationAddressesTokensIDsandExpiredDates(
          signers.addr1.address,
          "0x8888888888888888888888888888888888888888",
          "1"
        )
